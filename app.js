const STORAGE_KEY = "coinquest-db-v1";
...
function defaultDB() {
  return {
    users: [],
    leaderboardSeed: [
      { name: "CashFlowKai", xp: 1080, level: 9 },
      { name: "StudyGrindMia", xp: 920, level: 8 },
      { name: "SavvyJamal", xp: 810, level: 7 }
    ]
  };
}
...
function renderDashboard() {
  ...
  document.getElementById("daily-challenge").textContent = "Daily quest: Complete 2 lessons today for +25 bonus XP and a streak boost.";
...
function renderCourses() {
  ...
  catalog.courses.forEach((course, cIdx) => {
    ...
    node.querySelector(".course-lessons").textContent = `${progress.total} micro-quests`;
...
function answerQuestion(selected) {
  ...
  feedback.textContent = correct ? "✅ Nice! +30 XP unlocked" : "❌ Close! Check the explanation and crush the next one.";
...
function evaluateBadges() {
  ...
  if (completedCount >= 1) badgeSet.add("🚀 First Quest");
  if (completedCount >= 5) badgeSet.add("⚡ Momentum Mode");
  ...
  if (coursesCompleted >= 1) badgeSet.add("🏆 Course Conqueror");
...
function renderProgress() {
  ...
  badgeGrid.innerHTML = currentUser.badges.length
    ...
    : '<span class="muted">No badges yet — finish your first quest to unlock achievements.</span>';
}
...
authForm.addEventListener("submit", (e) => {
  ...
  if (user) {
    alert("Account already exists. Hit Log In.");
    return;
  }
...
document.getElementById("login-btn").addEventListener("click", () => {
  ...
  if (!user) {
    alert("Invalid credentials. Check email/password and try again.");
    return;
  }
  currentUser = user;
  setLoggedInUI(true);
...
document.getElementById("calc-budget").addEventListener("click", () => {
  ...
  document.getElementById("budget-result").textContent = remaining >= 0
    ? `You can stash $${remaining.toFixed(2)} this month.`
